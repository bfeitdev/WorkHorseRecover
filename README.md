...
- `v1.0.2` is prepared as the first release intended to publish validated standalone binaries for all five native targets.
...
Until the `v1.0.2` tag and release are actually published, the latest GitHub Release page may still point to `v1.0.0`, which does not contain standalone binaries. This README prepares the stable release download locations that the first standalone-binary release will use.
